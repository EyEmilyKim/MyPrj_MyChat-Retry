...
async function authenticateSocket(socket, next) {
  ...
  if (!cookieHeader) {
    return next(new Error('Socket Authentication error A - There is no cookie-header provided'));
  }
  // 쿠키 파싱
  ...
  // 토큰 확인 로직
  try {
    const tokenDecoded = await verifyToken(accessToken, 'AT');
    socket.decoded = tokenDecoded; // 소켓에 인증 정보 저장
    next(); // 다음 미들웨어로 이동
  } catch (error) {
    console.error('Token verification failed', error);
    return next(new Error('Socket Authentication error B - Token Verification failed'));
  }
}
